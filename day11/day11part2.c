...
#define TEST_ROWS 99
#define TEST_COLS 92
char testInput[TEST_ROWS][TEST_COLS] = 
{
...
char expectedRes1[TEST_ROWS][TEST_COLS] =
{
"#.##.##.##",
"#######.##",
"#.#.#..#..",
...
char expectedRes2[TEST_ROWS][TEST_COLS] =
{
"#.LL.L#.##",
"#LLLLLL.L#",
"L.L.L..L..",
...
char finalExpectedRes[TEST_ROWS][TEST_COLS] =
{
"#.#L.L#.##",
"#LLL#LL.L#",
"L.#.L..#..",
...
static inline bool mapsAreEqual(char input1[TEST_ROWS][TEST_COLS],
                                char input2[TEST_ROWS][TEST_COLS])
{
    size_t row;
    size_t col;
...
static inline size_t directionOccupied(const char map[TEST_ROWS][TEST_COLS],
                                       const int row,
                                       const int col,
                                       const int rowOffs,
                                       const int colOffs)
{
    ...
    while (!end)
    {
        ...
        if ((curRow < 0 || curRow >= TEST_ROWS) ||
            (curCol < 0 || curCol >= TEST_COLS))
        {
            end = true;
        }
        else
...
static char compResLetter(const char curMap[TEST_ROWS][TEST_COLS],
                          const int row,
                          const int col)
{
    int rowOffs;
    ...
    for (rowOffs = -1; rowOffs <= 1; rowOffs++)
    {
        for (colOffs = -1; colOffs <= 1; colOffs++)
        {
            if (rowOffs == 0 && colOffs == 0)
            {
                continue;
            }
            occupiedCount += directionOccupied(curMap,
                                               row,
                                               col,
                                               rowOffs,
                                               colOffs);
    ...
    char curLetter = curMap[row][col];
    if (curLetter == 'L' && occupiedCount == 0)
    {
        return '#'; // becomes occupied
    }
    else if (curLetter == '#' && occupiedCount >= 5)
    {
        return 'L'; // becomes empty
    }

    return curLetter;
}

static inline void printMap(const char map[TEST_ROWS][TEST_COLS])
{
    int row;
    int col;
    for (row = 0; row < TEST_ROWS; row++)
...
static void runIteration(const char curMap[TEST_ROWS][TEST_COLS],
                         char resMap[TEST_ROWS][TEST_COLS])
{
    int row;
    int col;
    for (row = 0; row < TEST_ROWS; row++)
    {
        for (col = 0; col < TEST_COLS; col++)
        {
            char resLetter = compResLetter(curMap,
                                           row,
                                           col);
            resMap[row][col] = resLetter;
        }
    }
}

static inline size_t countTotalOccupied(char map[TEST_ROWS][TEST_COLS])
{
    int row;
    int col;
    size_t count = 0;
...
int main(void)
{
    bool mapNotChanged = false;
    char curMap[TEST_ROWS][TEST_COLS];
    char resMap[TEST_ROWS][TEST_COLS];

    memcpy(curMap, testInput, sizeof(curMap));
    do
    {
        runIteration(curMap, resMap);
        mapNotChanged = mapsAreEqual(curMap, resMap);
        memcpy(curMap, resMap, sizeof(curMap));
    } while (!mapNotChanged);

    printf("Num occupied: %lu\n", countTotalOccupied(resMap));


#if 0
    if (!mapsAreEqual(resMap, finalExpectedRes))
    {
        printf("ERROR FINAL MAP\n");
    }
#endif
