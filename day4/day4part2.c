/*
 * day4
 * scan and check passport fields
 */
...
static inline bool getword(const char* str, 
                           const size_t len,
                           size_t* index, 
                           char* res)
{
...
static bool validPassport(const char* str)
{
    char field[50];
    char val[50];
    ...
    size_t index;
    size_t j;
    bool validFieldsFound[7] = { false };
    for (index = 0; index < len; index++)
    {
        if (!getword(str, len, &index, field))
        {
            break;
        }
        index++; // skip :
        if (!getword(str, len, &index, val))
        {
            break;
        }
        printf("Field, val: %s, %s\n", field, val);
        ...
        if (strcmp(field, "byr") == 0)
        {
            ...
            if (strlen(val) != 4)
            {
                validFieldsFound[0] = false;
                printf("  %s valid: %d\n", val, validFieldsFound[0]);
                continue;
            }
            sscanf(val, "%d", &year);
            if (year >= 1920 && year <= 2002)
            {
                validFieldsFound[0] = true;
            }
            else
            {
                validFieldsFound[0] = false;
        ...
        else if (strcmp(field, "iyr") == 0)
        {
            ...
            if (strlen(val) != 4)
            {
                validFieldsFound[1] = false;
                printf("  %s valid: %d\n", val, validFieldsFound[1]);
                continue;
            }
            sscanf(val, "%d", &year);
            if (year >= 2010 && year <= 2020)
            {
                validFieldsFound[1] = true;
            }
            else
            {
                validFieldsFound[1] = false;
        ...
        else if (strcmp(field, "eyr") == 0)
        {
            ...
            if (strlen(val) != 4)
            {
                validFieldsFound[2] = false;
                printf("  %s valid: %d\n", val, validFieldsFound[2]);
                continue;
            }
            sscanf(val, "%d", &year);
            if (year >= 2020 && year <= 2030)
            {
                validFieldsFound[2] = true;
            }
            else
            {
                validFieldsFound[2] = false;
        ...
        else if (strcmp(field, "hgt") == 0)
        {
            int height = 0;
            char unit[50];
            sscanf(val, "%d%s", &height, unit);
            printf("  height, unit: %d, %s\n", height, unit);
            if (strlen(unit) != 2 ||
                (strcmp(unit, "in") != 0 &&
                strcmp(unit, "cm") != 0))
            {
                validFieldsFound[3] = false;
                printf("  %s valid: %d\n", val, validFieldsFound[3]);
                continue;
            }
            if (strcmp(unit, "in") == 0)
            {
        ...
        else if (strcmp(field, "hcl") == 0)
        {
            ...
            if (val[0] != '#')
            {
                validFieldsFound[4] = false;
                printf("  %s valid: %d\n", val, validFieldsFound[4]);
                continue;
            }
            if (strlen(&val[1]) != 6)
            {
        ...
        {
            validFieldsFound[5] = false;
            if (strcmp(val, "amb") == 0)
            {
                validFieldsFound[5] = true;
            }
            else if (strcmp(val, "blu") == 0)
            {
                validFieldsFound[5] = true;
            }
            else if (strcmp(val, "brn") == 0)
            {
                validFieldsFound[5] = true;
            }
            else if (strcmp(val, "gry") == 0)
            {
                validFieldsFound[5] = true;
            }
            else if (strcmp(val, "grn") == 0)
            {
                validFieldsFound[5] = true;
            }
            else if (strcmp(val, "hzl") == 0)
            {
                validFieldsFound[5] = true;
            }
            else if (strcmp(val, "oth") == 0)
            {
                validFieldsFound[5] = true;
            }
            printf("  %s valid: %d\n", val, validFieldsFound[5]);
        }
        else if (strcmp(field, "pid") == 0)
        {
            ...
            if (strlen(val) != 9)
            {
                validFieldsFound[6] = false;
                printf("  %s valid: %d\n", val, validFieldsFound[6]);
                continue;
            }
            validFieldsFound[6] = true;
            for (i = 0; i < 9; i++)
            {
                if (val[i] < '0' || val[i] > '9')
                {
    ...
    for (j = 0; j < 7; j++)
    {
        if (!validFieldsFound[j])
        {
            printf("Overall valid: 0\n\n");
            return false;
        }
    }

    printf("Overall valid: 1\n\n");
    return true;
}
...
int main(void)
{
    ...
    for (i = 0; i < 265; i++)
    {
        size_t valid = (size_t)validPassport(input[i]);
        validCount += valid;
    }
    printf("Valid: %d\n", validCount);
    return 0;
