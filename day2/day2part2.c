/*
 * Day 2 - scan a set of passwords and see if they match the rules
 */
...
static bool validPassword(const int index1, const int index2, const char letter, const char* password)
{
    size_t i;
    bool valid = false;
    const int len = strlen(password);
    if (index1-1 < len && index2-1 < len)
    {
        char check1 = password[index1-1];
        char check2 = password[index2-1];
        bool matches1 = (check1 == letter);
        bool matches2 = (check2 == letter);
    ...
    {
        printf("Invalid indices for password %s: %d, %d\n", password, index1, index2);
    }
    return valid;
}
...
int main(void)
{
    ...
    for (i = 0; i < 1000; i++)
    {
        sscanf(input[i], "%d-%d %c: %s", &index1, &index2, &letter, password);
        validCount += validPassword(index1, index2, letter, password);
    }
