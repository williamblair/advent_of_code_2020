...
static bool validPassword(const int min, const int max, const char letter, const char* password)
{
    size_t i;
...
int main(void)
{
    ...
    for (i = 0; i < 1000; i++)
    {
        sscanf(input[i], "%d-%d %c: %s", &min, &max, &letter, password);
        validCount += validPassword(min, max, letter, password);
    }
