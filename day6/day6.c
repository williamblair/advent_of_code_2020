...
static inline int getNumPeople(const char* entry, const size_t len)
{
    size_t i;
    int numPeople = 0;
    for (i = 0; i < len; i++)
...
static inline void countOnePerson(const char* entry, 
                                  size_t* index, 
                                  bool* resArray)
{
    while (entry[*index] != ' ')
    {
        resArray[entry[*index] - 'a'] = true;
        (*index)++;
    }
    (*index)++; // skip the space char
}


static bool answeredQs[26];
static bool curAnswers[26];
static bool countEntry(const char* entry, 
                       int* numQuestions, 
                       int* numPeople)
{
    size_t i;
    ...
    *numQuestions = 0;
    for (i = 0; i < 26; i++)
    {
        *numQuestions += (size_t)answeredQs[i];
    }

    *numPeople = getNumPeople(entry, len);
...
{
    size_t i;
    int numQuestions, numPeople;
    int questionsSum = 0;
    for (i = 0; i < 472; i++)
    {
        countEntry(input[i], &numQuestions, &numPeople);
        printf("Num questions, num people: %d, %d\n", numQuestions, numPeople);
        questionsSum += numQuestions;
    }
    printf("Sum: %d\n", questionsSum);
    return 0;
}
