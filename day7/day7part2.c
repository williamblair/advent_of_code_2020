...
static inline void freeRules()
{
    size_t i;
    for (i = 0; i < 594; i++)
    //for (i = 0; i < 7; i++)
    {
        if (rules[i].subRules != NULL)
        {
...
static inline void getColor(const char* input,
                            size_t* index,
                            char* result)
{
    size_t spacesCount = 0;
    while (*index < strlen(input) && spacesCount < 2)
    {
        *result = input[*index];
        if (*result == ' ')
        {
...
static inline int getNumSubRules(const char* input)
{
    size_t i = 0;
    int numrules = 1;
    const size_t len = strlen(input);
    if (input[0] == 'n' && input[1] == 'o')
    {
        // "no other bags" -> 0
        return 0;
    }
...
static inline void parseRule(HeadRule* head, const char* input)
{
    int numRules;
    size_t index = 0;
    size_t i;
    getColor(input, &index, head->color);
    //printf("Head Color: %s\n", head->color);

    index += strlen("bags contain ");
    numRules = getNumSubRules(&input[index]);
    //printf("Num rules: %d\n", numRules);
    if (numRules > 0)
    {
        head->subRules = (Rule*)malloc(numRules*sizeof(Rule));
        head->numSubRules = numRules;
    }
    else
    {
        head->subRules = NULL;
    ...
    {
        sscanf(&input[index], "%d", &head->subRules[i].number);
        //if (testnum >= 10)
        //{
        //    printf("  WARNING - NUM > 10!!!!!!!!!\n");
        //}
        index += 2;
        getColor(input, &index, head->subRules[i].color);
        //printf("sub num: %d\n", head->subRules[i].number);
        //printf("sub color: %s\n", head->subRules[i].color);
        while (input[index] != ' ' && input[index] != '.' && index < strlen(input))
        {
            index++;
        }
...
static HeadRule* findHeadRule(const char* color)
{
    size_t i;
    for (i = 0; i < 594; i++)
    //for (i = 0; i < 7; i++)
...
static bool containsShinyGold(HeadRule* rule)
{
    size_t i = 0;
    Rule* subrule = NULL;
    bool containsGold = false;
    if (rule->numSubRules == 0)
    {
        return false;
    }
    for (i = 0; i < rule->numSubRules; i++)
    {
        subrule = &rule->subRules[i];
        if (strcmp(subrule->color, "shiny gold ") == 0)
        {
            printf("Found shiny gold in subrule\n");
            return true;
        }
        else
        {
            // check subrules
            HeadRule* r = findHeadRule(subrule->color);
            if (r == NULL)
            {
                printf("  FAILED TO FIND HEAD RULE WITH COLOR: %s\n", subrule->color);
                return false;
            }
            else
            {
                containsGold = containsShinyGold(r);
                if (containsGold)
                {
                    return true;
                }
...
static size_t getBagCount(HeadRule* rule)
{
    size_t i;
    size_t count = 0;
    for (i = 0; i < rule->numSubRules; i++)
    {
        HeadRule* subRule = findHeadRule(rule->subRules[i].color);
        if (subRule == NULL)
        {
            printf("FAILED TO FIND SUB RULE FOR %s\n", rule->subRules[i].color);
            return 0;
        }
        else
        {
            count += rule->subRules[i].number + (rule->subRules[i].number * getBagCount(subRule));
        }
    }
...
int main(void)
{
    HeadRule* shinyGoldRule = NULL;
    size_t shinyGoldCount = 0;
    size_t i;
    for (i = 0; i < 594; i++)
    {
        parseRule(&rules[i], input[i]);
    }
//    for (i = 0; i < 7; i++)
//    {
//        parseRule(&rules[i], testInput[i]);
 //   }

    shinyGoldRule = findHeadRule("shiny gold ");
    if (shinyGoldRule == NULL)
    {
        printf("Failed to find shiny gold\n");
    }
    else
    {
        shinyGoldCount = getBagCount(shinyGoldRule);
        printf("shiny gold count: %d\n", shinyGoldCount);
    }
