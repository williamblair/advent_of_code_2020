/*
 * day 14 - ship docking bitmasking
 */
...
#if 1
#define INPUT_LEN 580
const char* input[580] = 
{
...
static inline void getMaxAndMinAddr(uint64_t* maxRes, uint64_t* minRes)
{
    uint64_t max = 0;
...
public:
    ...
    {
        memcpy(val, other.val, BIT_LEN+1);
    }
...
std::map<uint64_t,MapEntry> mem;
static inline char* getMemEntry(const uint64_t index)
{
    MapEntry dummy;
    if (mem.count(index) == 0)
    {
        mem.insert(std::pair<uint64_t, MapEntry>(index, dummy));
    }
    return (char*)&mem[index].val;
}

static inline void setMask(const char* entry)
{
    sscanf(entry, "mask = %s", mask);
}

static inline void setMemAddr(const uint64_t index, const uint64_t val)
{
    MapEntry memEntry;
    uint64_t i;
    uint64_t power = 1;
    uint64_t memIndex = BIT_LEN-1;
    for (i = 0; i < BIT_LEN; i++, memIndex--)
    {
        char curBit;
        curBit = ((val & power) > 0) ? '1' : '0';
        memEntry.val[memIndex] = curBit;
        power <<= 1;
    }
    mem[index] = memEntry;
    printf("Set index %llu with val %llu (%s)\n", index, val, memEntry.val);
}

static inline void setMemAddrFromStr(const char* addr, const uint64_t setVal)
{
    uint64_t memNum = 0;
    uint64_t power = 1;
    uint64_t i;
    ...
    for (i = 0; i < BIT_LEN; i++, memIndex--)
    {
        memNum += power * (addr[memIndex] == '1');
        power <<= 1;
    }
    setMemAddr(memNum, setVal);
}

char maskedAddr[BIT_LEN+1];
static inline void parseAddr(const uint64_t addr)
{
    uint64_t i;
    uint64_t power = 1;
    uint64_t memIndex = BIT_LEN - 1;
    memset(maskedAddr, 0, sizeof(maskedAddr));
    for (i = 0; i < BIT_LEN; i++, memIndex--)
    {
        maskedAddr[memIndex] = ((addr & power) > 0) ? '1' : '0';
        if (mask[memIndex] == '1' ||
            mask[memIndex] == 'X')
        {
            maskedAddr[memIndex] = mask[memIndex];
        }
        power <<= 1;
    }
}

static inline uint64_t getNumAddrs()
{
    uint64_t amount = 1;
    size_t i;
    for (i = 0; i < BIT_LEN; i++)
...
static inline uint64_t getXCount()
{
    uint64_t amount = 0;
    size_t i;
    for (i = 0; i < BIT_LEN; i++)
...
static inline void getXIndices(uint64_t* indices)
{
    size_t i;
...
static void applyParsedAddr(const uint64_t val)
{
    uint64_t i;
    uint64_t j;
    uint64_t flipInterval = 1;
    char curBit = '0';
    char* curAddrStr = NULL;
    char* addrs = NULL;
    uint64_t* xIndices = NULL;
    uint64_t numAddrs = getNumAddrs();
    uint64_t xCount = getXCount();
    addrs = (char*)malloc(numAddrs * BIT_LEN);
    xIndices = (uint64_t*)malloc(xCount * sizeof(uint64_t));
    
    getXIndices(xIndices);
    
    for (i = 0; i < numAddrs; i++)
    {
        curAddrStr = &addrs[i * BIT_LEN];
        memcpy(curAddrStr, maskedAddr, BIT_LEN);
    }

    for (i = 0; i < xCount; i++)
    {
        uint64_t bitIndex = xIndices[i];
        curBit = '0';
        for (j = 0; j < numAddrs; j++)
        {
            curAddrStr = &addrs[j * BIT_LEN];
            curAddrStr[bitIndex] = curBit;
            if (j % flipInterval == 0)
            {
                curBit = (curBit == '0') ? '1' : '0';
            }
    ...
    {
        char str[BIT_LEN+1];
        memcpy(str, &addrs[i * BIT_LEN], BIT_LEN);
        str[BIT_LEN] = '\0';
        printf("%s\n", str);
    }
#endif
    ...
    for (i = 0; i < numAddrs; i++)
    {
        setMemAddrFromStr(&addrs[i * BIT_LEN], val);
    }
...
static void handleMemset(const char* entry)
{
    uint64_t addr = 0;
    uint64_t val = 0;
    sscanf(entry, "mem[%llu] = %llu", &addr, &val);
    printf("Addr, val: %llu, %llu\n", addr, val);
    parseAddr(addr);
    applyParsedAddr(val);
}

static inline uint64_t addrToVal(const uint64_t addr)
{
    uint64_t val = 0;
    uint64_t power = 1;
    uint64_t i;
    uint64_t memIndex = BIT_LEN-1;
    char* memEntry = getMemEntry(addr);
    for (i = 0; i < BIT_LEN; i++, memIndex--)
    {
        val += power * (memEntry[memIndex] == '1');
        power <<= 1;
    }
    return val;
}
...
int main(void)
{
    ...
    for (i = 0; i < INPUT_LEN; i++)
    {
        ...
        {
            printf("ERROR - unhandled line: %s\n", entry);
            break;
        }
    }

    uint64_t sum = 0;
    for (auto& memEntry : mem)
    {
        uint64_t val = addrToVal(memEntry.first);
        sum += val;
        if (val != 0)
        {
            printf("Adding val, new total: %llu, %llu\n", val, sum);
        }
    }
    printf("Sum: %llu\n", sum);
