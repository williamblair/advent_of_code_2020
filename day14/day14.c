...
static inline void setMemAddr(const uint64_t index, const uint64_t val)
{
    ...
    char* memEntry = getMemEntry(index);
    for (i = 0; i < BIT_LEN; i++, memIndex--)
    {
        ...
        //printf("  curBit: %c\n", curBit);
        memEntry[memIndex] = curBit;
        if (mask[memIndex] != 'X')
        {
            memEntry[memIndex] = mask[memIndex];
        }
        power <<= 1;
    }
}
...
int main(void)
{
    ...
    uint64_t minAddr = 0;
    uint64_t maxAddr = 0;
    getMaxAndMinAddr(&maxAddr, &minAddr);
    
    printf("Max, min addr: %d, %d\n", maxAddr, minAddr);
    mem = (char*)malloc( (maxAddr+1) * BIT_LEN );
    memset(mem, 0, (maxAddr+1) * BIT_LEN );
